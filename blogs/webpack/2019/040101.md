...
> 业务开发时，一般不是设置 hotOnly，这样才能实时显示最新代码和更改效果
...
1. 如果我们在 `a.js` 引用了一个 `utils.js` 的里面的方法，而这个方法需要借助 `jquery`， `a.js` 引用了`jquery`，而 `utils.js` 没有，这时候是会报错的，所以可以借助 `webpack.ProvidePlugin` 全局在使用 $ 的地方引用 `jquery`。
