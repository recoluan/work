...
`babel-plugin-dynamic-import-webpack` 不支持魔法注释，所以要换成 `@babel/plugin-syntax-dynamic-import`，这个时候会打包生成 `vendors~lodash.js`，如果需要修改打包后的名字：

```js
module.exports = {
  optimization: : {
    splitChunks: {
    chunks: "async", // async 异步， all 全部，但是同步还需要配置 cacheGroups
    minSize: 30000, // 可以处理依赖的最小值
    minChunks: 1, // 可以处理依赖的最大值
    maxAsyncRequests: 5,
